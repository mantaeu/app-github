...
export class PDFService {
  ...
  // Helper method to draw a simple table with white background and black text
  private static drawTable(doc: any, startX: number, startY: number, width: number, headers: string[], rows: any[][], options: any = {}) {
    ...
    // Draw header with white background and black text
    doc.rect(startX, currentY, width, headerHeight).fill('#FFFFFF').stroke(borderColor);
    ...
    // Draw rows with white background and black text
  ...
  // Create individual salary slip PDF with white background, black text, orange logo
  private static createSalarySlipPDF(salary: any, user: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.rect(leftMargin, 40, pageWidth, 100).fill('#FFFFFF').stroke('#000000');
        ...
        // Document Title in orange
        doc.fontSize(24).fillColor('#ff6600').font('Helvetica-Bold');
        doc.text('SALARY SLIP', pageWidth - 150, 65);
        
        // Document Info Box with white background
        doc.rect(pageWidth - 140, 85, 120, 45).fill('#FFFFFF').stroke('#000000');
        doc.fontSize(9).fillColor('#000000').font('Helvetica');
        ...
        const salaryRows = [
          ...
          ['Deductions', `-${(salary.deductions || 0).toFixed(2)} DH`, 'Deduction']
        ];

        currentY = this.drawTable(doc, leftMargin, currentY, pageWidth, salaryHeaders, salaryRows);

        // Net Salary Box with white background and orange border
        currentY += 20;
        doc.rect(leftMargin, currentY, pageWidth, 40).fill('#FFFFFF').stroke('#ff6600');
        doc.fontSize(16).fillColor('#000000').font('Helvetica-Bold');
        doc.text('NET SALARY', leftMargin + 20, currentY + 12);
        doc.fontSize(20).fillColor('#ff6600');
        ...
        if (salary.presentDays !== undefined) {
          ...
          const attRows = [
            ...
            ['Days Present', `${salary.presentDays || 0}`, 'Days attended'],
            ['Days Absent', `${salary.absentDays || 0}`, 'Days missed'],
        ...
        // HR Signature Box
        doc.rect(leftMargin, currentY, (pageWidth / 2) - 10, 80).fill('#FFFFFF').stroke('#000000');
        ...
        doc.rect(leftMargin + (pageWidth / 2) + 10, currentY, (pageWidth / 2) - 10, 80).fill('#FFFFFF').stroke('#000000');
        doc.fontSize(12).fillColor('#000000').font('Helvetica-Bold');
        ...
        currentY += 110;
        doc.rect(leftMargin, currentY, pageWidth, 40).fill('#FFFFFF').stroke('#000000');
        doc.fontSize(9).fillColor('#000000').font('Helvetica');
  ...
  // Create individual receipt PDF with white background, black text, orange logo
  private static createReceiptPDF(receipt: any, user: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Receipt Info Box with white background
        doc.rect(pageWidth - 140, 85, 120, 45).fill('#FFFFFF').stroke('#000000');
        doc.fontSize(9).fillColor('#000000').font('Helvetica');
        ...
        // Amount Section with white background and orange border
        currentY += 30;
        doc.rect(leftMargin, currentY, pageWidth, 60).fill('#FFFFFF').stroke('#ff6600');
        doc.fontSize(16).fillColor('#000000').font('Helvetica-Bold');
        doc.text('TOTAL AMOUNT', leftMargin + 20, currentY + 15);
        doc.fontSize(36).fillColor('#ff6600');
        ...
        currentY += 20;
        doc.rect(leftMargin, currentY, pageWidth, 60).fill('#FFFFFF').stroke('#000000');
  ...
  // Create all salaries PDF with white background, black text, orange logo
  private static createAllSalariesPDF(salaryRecords: any[]): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Header with orange logo and black text
  ...
  // Create all receipts PDF with white background, black text, orange logo
